  ...
  return (
    <div className="mb-6">
          ...
          <div key={label} className="flex-1 text-center">
            <div
              className={classNames(
                ...
                idx === currentStep ? "text-[#0061a1]" : "text-gray-500"
              )}
              title={label}
            >
              {label}
      ...
      <div className="w-full bg-gray-200 rounded-full h-2">
        <div
          className="bg-[#0061a1] h-2 rounded-full transition-all"
      ...
      <div className="rounded-2xl border-2 border-dashed border-gray-300 p-4 text-center">
        <p className="text-md text-gray-600">
          ...
          <span className="font-medium ml-2">
            <label className="text-md text-blue-600 underline cursor-pointer shadow-sm">
              <input
                type="file"
                accept={accept}
                multiple={multiple}
...
export default function ChinaAdmissionForm() {
  ...
  const updateFiles = (name, newFiles, append = false) =>
    setData((prev) => ({
      ...prev,
      [name]: append ? [...(prev[name] || []), ...newFiles] : newFiles,
    }));
  ...
  const updateEducation = (index, field, value) => {
    setData((prev) => {
      const next = Array.isArray(prev.educationHistory)
  ...
  const updateFamily = (index, field, value) => {
    setData((prev) => {
      const next = Array.isArray(prev.familyMembers)
        ? [...prev.familyMembers]
        : [];
      next[index] = { ...next[index], [field]: value };
  ...
  const validateEducationEntry = (edu) => {
    ...
    if (!edu.schoolAttended || !edu.schoolAttended.trim())
      errs.push("School Attended is required");
    if (!edu.location || !edu.location.trim())
      errs.push("Location is required");
    if (!edu.major || !edu.major.trim())
      errs.push("Major of study is required");
  ...
  const validateFamilyMemberEntry = (fam) => {
    ...
    if (!fam.familyMemberName || !fam.familyMemberName.trim())
      errs.push("Name is required");
    if (!fam.nationality || !fam.nationality.trim())
      errs.push("Nationality is required");
    if (!fam.mobileNumber || !fam.mobileNumber.trim())
      errs.push("Mobile Number is required");
    if (!fam.email || !fam.email.trim()) errs.push("Email is required");
    if (!fam.profession || !fam.profession.trim())
      errs.push("Profession is required");
    if (!fam.workPlace || !fam.workPlace.trim())
      errs.push("Work Place is required");
    if (!fam.relationship || !fam.relationship.trim())
      errs.push("Relationship is required");
    return errs;
  };
  ...
  const validateStep = (s = step) => {
        ...
        if (!Array.isArray(v) || v.length <= 1) {
          ...
          v.forEach((fam, idx) => {
            ...
            if (e.length) {
              famErrs[idx] = e;
              e.forEach((m) =>
                errs.push(`Family Member entry ${idx + 1}: ${m}`)
              );
            }
          });
        }
    ...
    if (s === 0) {
      ...
      if (data.phone && !validators.phone(data.phone))
        errs.push("Phone number looks invalid!");
  ...
  const onSubmit = async (e) => {
    ...
    formData.append(
      "relationshipWithApplicant",
      data.relationshipWithApplicant
    );
        ...
        <div className="mx-auto max-w-6xl px-4">
          <div className="mb-8 text-center">
            <h1 className="text-2xl sm:text-3xl font-bold tracking-tight">
              Admission Application – Study in China
            </h1>
            ...
            <p className="text-gray-600 mt-2">
              Complete the steps below. Your progress is saved locally in your
              browser.
            </p>
          </div>
                  ...
                  <div className="space-y-4">
                    ...
                    {Array.isArray(data.educationHistory) &&
                      data.educationHistory.map((edu, idx) => (
                        <div
                          key={idx}
                          className="grid grid-cols-1 md:grid-cols-2 gap-4 border rounded-xl p-3"
                        >
                          <TextField
                            ...
                            required
                            onChange={(n, v) =>
                              updateEducation(idx, "schoolAttended", v)
                            }
                          ...
                          <TextField
                            ...
                            required
                            onChange={(n, v) =>
                              updateEducation(idx, "location", v)
                            }
                          ...
                          <TextField
                            ...
                            onChange={(n, v) =>
                              updateEducation(idx, "dateAttended", v)
                            }
                          />
                          ...
                          <SelectField
                            ...
                            onChange={(n, v) =>
                              updateEducation(idx, "degreeAwarded", v)
                            }
                            options={[
                              "Junior High School",
                              "Senior High School",
                      ...
                      data.familyMembers.map((fam, idx) => (
                        ...
                        >
                          <TextField
                            ...
                            onChange={(n, v) =>
                              updateFamily(idx, "familyMemberName", v)
                            }
                          ...
                          <TextField
                            ...
                            onChange={(n, v) =>
                              updateFamily(idx, "nationality", v)
                            }
                            placeholder={"Enter Nationality"}
                          />
                          ...
                          <TextField
                            ...
                            onChange={(n, v) =>
                              updateFamily(idx, "mobileNumber", v)
                            }
                            type="tel"
                          ...
                          <TextField
                            ...
                            onChange={(n, v) =>
                              updateFamily(idx, "relationship", v)
                            }
                            placeholder={"Enter Relationship"}
                          />
                ...
                <section className="space-y-4">
                      ...
                      <div className="grid grid-cols-1 gap-3">
                        {data.educationHistory.map((edu, i) => (
                          <div key={i} className="rounded-xl border p-3">
                            ...
                            <div className="text-xs text-gray-500">
                              {edu.dateAttended
                                ? `${edu.dateAttended} • ${edu.field || ""}`
                                : "—"}
                            </div>
                            <div className="text-xs text-gray-500">
                              {edu.degreeAwarded
                                ? `${edu.degreeAwarded} • ${edu.field || ""}`
                                : "—"}
                            </div>
                          </div>
                        ))}
                      ...
                      <div className="grid grid-cols-1 gap-3">
                        {data.familyMembers.map((fam, i) => (
                          <div key={i} className="rounded-xl border p-3">
                            ...
                            <div className="text-xs text-gray-500">
                              {fam.nationality
                                ? `${fam.nationality} • ${fam.field || ""}`
                                : "—"}
                            </div>
                            <div className="text-xs text-gray-500">
                              {fam.mobileNumber
                                ? `${fam.mobileNumber} • ${fam.field || ""}`
                                : "—"}
                            </div>
                            <div className="text-xs text-gray-500">
                              {fam.email
                            ...
                            <div className="text-xs text-gray-500">
                              {fam.profession
                                ? `${fam.profession} • ${fam.field || ""}`
                                : "—"}
                            </div>
                            <div className="text-xs text-gray-500">
                              {fam.workPlace
                                ? `${fam.workPlace} • ${fam.field || ""}`
                                : "—"}
                            </div>
                            <div className="text-xs text-gray-500">
                              {fam.relationship
                                ? `${fam.relationship} • ${fam.field || ""}`
                                : "—"}
                            </div>
                          </div>
                        ))}
                  ...
                  {submitted && (
                    <div className="rounded-xl border border-green-200 bg-gray-50 p-4 text-black">
                      ...
                      <p>
                        1． 申请表中所填写的内容和提供的材料真实无误； All
                        information and materials given in this form are true
                        and correct.
                      </p>
                      <p>
                        2．
                        在华其间，遵守中国的法律、法规，不从事任何危害中国秩序的、与本人来华学习身份不相符的活动；
                        During my stay in China, I shall abide by the laws and
                        decrees of the Chinese government, and will not
                        participate in any activities, which are deemed to be
                        adverse to the social order in China and inappropriate
                        to the capacity as a scholar or a student.
                      </p>
                      <p>
                        3．
                        在学期间，遵守学校的校纪、校规，全力投入学习和研究工作。尊重学校的教学安排；
                        During my study in China, I shall observe the rules and
                        regulations of the university, and will concentrate on
                        my studies and researches, and will follow the teaching
                        programs made by the university.
                      </p>
                      <p>
                        4． 按规定修完学业，按期回国，不无故在华滞留； I shall
                        return to my home country as soon as I complete my
                        scheduled program in China, and will not extend my stay
                        without valid reasons.
                      </p>
                      <p>
                        5．
                        我一到学校就想办理所有登记手续。我将在抵达后30天内向公安局申请居住证，否则我必须支付延误的罚款。
                        I’d like to go through all the procedures of
                        registration to the university as soon as I arrive. I
                        will apply within thirty days of arrival for the
                        residency permit to the Public Security Bureau otherwise
                        I have to pay the forfeit for the delay.
                        <p>
                          申请人签名/Signature of the applicant:____________
                          <br />
                          日期/Date:_____________ 无此签名，申请无效/The
                          application is invalid without the applicant’s
                          signature
                        </p>
                      </p>

                      <h4>
                        申请人在递送本申请表的同时，请提交/Please send with this
                        form
                      </h4>
                      <p>
                        学历生所需材料Relative documents required for Degree and
                        Scholar applicants:
                        <p>
                          （1） 护照复印件/One photocopy of your passport(photo
                          page)
                        </p>
                        <p>
                          （2） 最后学历证明/An official certificate of your
                          highest education (or notarized photocopy)
                        </p>
                        <p>
                          （3） 学习成绩单/Official academic transcripts (or
                          notarized photocopy)
                        </p>
                        <p>
                        ...
                        <p>
                          （5） 家庭收入证明/ An official Certificate of your
                          sponsor’s true family financial situation;
                        </p>
                        <p>
                          （6） 健康证明/ Certificate of your health examination
                          records 无论申请人是否被录取，上述申请材料恕不退还
                          Whether the candidate is accepted or not, all the
                          application materials will not be returned
                        </p>
                        <b className="font-semibold text-lg">
                          Notice: Please fill in this form in capital letter.
                          Duplication of this form is available.
                        </b>
                        <p>
                          注意事项/Note:
                          申请人须用中文填写或用英文印刷体填写，其它文字或缺项填写的申请表无效。
                          This form is to be completed in Chinese or English
                          (print) only. An incomplete application or complete in
                          language other than Chinese or English is invalid.
                        </p>
                      </p>
                    </div>
                  )}
