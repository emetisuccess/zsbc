   ...
   <div className="flex items-center justify-between mb-2">
    {STEPS.map((label, idx) => (
     <div key={label} className="flex-1 text-center">
      <div
       className={classNames("text-xs sm:text-sm font-medium truncate", idx === currentStep ? "text-[#0061a1]" : "text-gray-500")}
       title={label}
      >
       {label}
      </div>
    ...
    {options.map((opt) => (
     <label
      ...
      className={classNames(
       ...
       value === opt ? "border-blue-500 ring-2 ring-blue-200" : "border-gray-300",
      )}
     >
      <input
       type="radio"
...
function FileField({ label, name, files = [], onFilesChange, accept, multiple = true, required }) {
 const previews = useMemo(
  () =>
   Array.isArray(files)
    ? files.map((file) => ({
       file,
       url: file && file.type && file.type.startsWith("image/") ? URL.createObjectURL(file) : null,
      }))
    : [],
  [files],
 );
 ...
 const handleFileSelect = (e) => {
  ...
  if (oversized.length > 0) {
   ...
   toast ? toast.error(`The following files exceed 500KB: ${names}`) : alert(`The following files exceed 500KB: ${names}`);
  ...
  <div>
   <div className="flex items-center justify-between">
    ...
    <label className="text-xs text-blue-600 underline cursor-pointer">
     <input type="file" accept={accept} multiple={multiple} className="hidden" onChange={handleFileSelect} />
     Add files
    </label>
   </div>
     ...
     {previews.map((p, idx) => (
      <li key={idx} className="relative group">
       <div className="rounded-xl border p-2 flex flex-col items-center justify-center h-32 overflow-hidden">
        {p.url ? (
         <img src={p.url} alt={p.file.name} className="object-cover w-full h-full rounded-lg" />
        ) : (
         <div className="text-center text-xs">
          <span className="block font-medium truncate">{p.file.name}</span>
          <span className="text-gray-500">{(p.file.size / 1024).toFixed(0)} KB</span>
         </div>
        )}
       </div>
       <button
...
export default function ChinaAdmissionForm() {
 ...
 const updateFiles = (name, newFiles, append = false) =>
  setData((prev) => ({ ...prev, [name]: append ? [...(prev[name] || []), ...newFiles] : newFiles }));
 ...
 const addEducation = () => {
  setData((prev) => ({
   ...
   educationHistory: [
    ...
    { schoolAttended: "", location: "", major: "", dateAttended: "", degreeAwarded: "" },
   ],
  }));
 };
 ...
 const addFamilyMember = () => {
  setData((prev) => ({
   ...
   familyMembers: [
    ...(prev.familyMembers || []),
    { familyMemberName: "", nationality: "", mobileNumber: "", email: "", profession: "", workPlace: "", relationship: "" },
   ],
  }));
 };
   ...
   <div className="min-h-screen bg-gray-50 py-10">
    ...
    <div className="mx-auto max-w-6xl px-4">
     <div className="mb-8 text-center">
      <h1 className="text-2xl sm:text-3xl font-bold tracking-tight">Admission Application – Study in China</h1>
      <small className="text-red-600 font-semibold underline text-md">
       Fill your information as it is on your passport datapage. Thanks.
      </small>
      <p className="text-gray-600 mt-2">Complete the steps below. Your progress is saved locally in your browser.</p>
     </div>
      ...
      >
       {step === 0 && (
        <section className="grid grid-cols-1 md:grid-cols-3 gap-4">
         <TextField
          ...
          required
          name="majorFirstChoice"
          placeholder="Enter First Choice"
          value={data.majorFirstChoice}
          onChange={update}
         />

         <TextField
          label="Major Second Choice (optional)"
          name="majorSecondChoice"
          value={data.majorSecondChoice}
         ...
         <TextField
          ...
          value={data.dob}
          onChange={update}
          placeholder={"Enter accurate date of birth"}
         />
         ...
         <TextField
          ...
          onChange={update}
          placeholder={"Enter Chinese name if any"}
         />
         <TextField
          required
        ...
        <div>
         <section className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <SelectField
           ...
           name="rateYourChinese"
           value={data.rateYourChinese}
           onChange={update}
           options={["Good", "Average", "Better", "Very Good", "None"]}
          />

          <SelectField
           required
           label="Rate Your English"
           name="rateYourEnglish"
           value={data.rateYourEnglish}
           onChange={update}
           options={["Good", "Average", "Better", "Very Good", "None"]}
          />
          ...
          <SelectField
           ...
           onChange={update}
           options={["None Degree Program", "Undergraduate", "Master's", "PhD"]}
          />
         ...
         <p className="border border-l-8 py-0.5 w-6/12 px-2 border-l-[#0061a1] mt-16 mb-2">
          Person or Agency to Action on your behalf in China
         </p>
         <div className="grid md:grid-cols-1 gap-2">
          <TextField label="Enter Agent" required name="agent" placeholder={"Enter Agent"} value={data.agent} onChange={update} />
         </div>
         <p className="border border-l-8 py-0.5 w-6/12 px-2 border-l-[#0061a1] mt-16 mb-2">
          Comment of Sponsor or Recommended Party
         </p>
         <div className="grid md:grid-cols-1 gap-2">
          <TextArea
           label="Enter Comment"
           ...
           onChange={update}
          />
          <TextField label="Enter Date" type="date" required name="signedDate" value={data.signedDate} onChange={update} />
         </div>
        </div>
       )}
       {step === 2 && (
           ...
           data.educationHistory.map((edu, idx) => (
            <div key={idx} className="grid grid-cols-1 md:grid-cols-2 gap-4 border rounded-xl p-3">
             <TextField
              ...
              required
              onChange={(n, v) => updateEducation(idx, "schoolAttended", v)}
             ...
             <TextField
              ...
              value={edu.location}
              required
              onChange={(n, v) => updateEducation(idx, "location", v)}
             ...
             <TextField
              ...
              value={edu.major}
              required
              onChange={(n, v) => updateEducation(idx, "major", v)}
             ...
             <TextField
              ...
              onChange={(n, v) => updateEducation(idx, "dateAttended", v)}
             />
             ...
             <SelectField
              ...
              name={`degreeAwarded-${idx}`}
              value={edu.degreeAwarded}
              onChange={(n, v) => updateEducation(idx, "degreeAwarded", v)}
              options={[
               "Junior High School",
               "Senior High School",
               "GCE",
             ...
             <div className="md:col-span-2 flex items-center gap-3">
              {data.educationHistory.length > 1 && (
               <button type="button" onClick={() => removeEducation(idx)} className="rounded-2xl border px-3 py-2 text-sm">
                Remove
               </button>
              )}
              {idx === data.educationHistory.length - 1 && (
               <button
                ...
                className="rounded-2xl bg-[#0061a1] px-3 py-2 text-sm font-semibold text-white"
               >
                Add another
               </button>
              )}
          ...
          <div className="space-y-6">
           ...
           <div className="grid md:grid-cols-2 gap-2">
            ...
            <TextField
             ...
             onChange={update}
            />
            <TextField type="date" label="Date Started" required name="dateStarted" value={data.dateStarted} onChange={update} />
           </div>
          </div>
         )}
        </section>
        ...
        <section className="space-y-6">
         <div className="space-y-4">
          ...
          {Array.isArray(data.familyMembers) &&
           data.familyMembers.map((fam, idx) => (
            <div key={idx} className="grid grid-cols-1 md:grid-cols-2 gap-4 border rounded-xl p-3">
             <TextField
              label="Family Member Name"
              required
              name={`familyMemberName-${idx}`}
              value={fam.familyMemberName}
              onChange={(n, v) => updateFamily(idx, "familyMemberName", v)}
             ...
             <TextField
              ...
              onChange={(n, v) => updateFamily(idx, "nationality", v)}
              placeholder={"Enter Nationality"}
             />
             ...
             <TextField
              label="Mobile Number"
              required
              name={`mobileNumber-${idx}`}
              value={fam.mobileNumber}
              onChange={(n, v) => updateFamily(idx, "mobileNumber", v)}
              type="tel"
             ...
             <TextField
              ...
              type="email"
              value={fam.email}
              onChange={(n, v) => updateFamily(idx, "email", v)}
              placeholder={"Enter Email e.g example@gmail.com"}
             />
             ...
             <TextField
              ...
              name={`profession-${idx}`}
              type="text"
              value={fam.profession}
              onChange={(n, v) => updateFamily(idx, "profession", v)}
             ...
             <TextField
              ...
              type="text"
              value={fam.workPlace}
              onChange={(n, v) => updateFamily(idx, "workPlace", v)}
             ...
             <TextField
              ...
              onChange={(n, v) => updateFamily(idx, "relationship", v)}
              placeholder={"Enter Relationship"}
             />

             <div className="md:col-span-2 flex items-center gap-3">
              {data.familyMembers.length > 1 && (
               <button type="button" onClick={() => removeFamilyMember(idx)} className="rounded-2xl border px-3 py-2 text-sm">
                Remove
               </button>
              )}
       ...
       {step === 4 && (
        <section className="space-y-6">
         <FileField
          required
          label="International Passport Photo (recent)"
          name="passportphoto"
         ...
         <FileField
          required
          label="Applicant Signature (Sign on paper and snap)"
          name="applicantSignature"
          files={data.applicantSignature}
          onFilesChange={updateFiles}
          accept="image/*"
          multiple={false}
         />
         ...
         <FileField
          required
          label="Sponsor's Signature (Sign on paper and snap)"
          name="sponsorSignature"
          files={data.sponsorSignature}
          onFilesChange={updateFiles}
          accept="image/*"
          multiple={false}
         />
         ...
         <FileField
          ...
          files={data.recommendationLetters}
          onFilesChange={updateFiles}
          accept="image/*,application/pdf"
          multiple={true}
         />
         ...
         <FileField
          label="Medical Report (pdf format)"
          name="medicalReport"
          files={data.medicalReport}
          onFilesChange={updateFiles}
         ...
         <FileField
          label="Bachelor/Masters Certificate (pdf format)"
          name="certificates"
         ...
         <FileField
          label="Bank Statement (pdf format)"
          name="bankStatement"
          files={data.bankStatement}
          onFilesChange={updateFiles}
          accept="application/pdf"
          multiple={true}
         />
         ...
         <FileField
          label="Non-Criminal Record (pdf format)"
          name="nonCriminalRecord"
          files={data.nonCriminalRecord}
          onFilesChange={updateFiles}
          accept="application/pdf"
          multiple={true}
         />
         ...
         <FileField
          label="English Language Proficiency Certificate (Non-English Speaking Countries)"
          name="englishCertificate"
          files={data.englishCertificate}
          onFilesChange={updateFiles}
          accept="application/pdf"
          multiple={true}
         />
         ...
         <FileField
          label="Research Proposal (doc format)"
          accept=".pdf, .doc, .docx"
          name="researchProposal"
          files={data.researchProposal}
          onFilesChange={updateFiles}
          multiple={true}
         />
        </section>
       ...
       {step === 5 && (
        <section className="space-y-4">
         ...
         {submitted && (
          <div className="rounded-xl border border-green-200 bg-gray-50 p-4 text-black">
           <h4>申请人保证/I hereby affirm: </h4>
           <p>
            1． 申请表中所填写的内容和提供的材料真实无误； All information and materials given in this form are true and correct.
           </p>
           <p>
            2． 在华其间，遵守中国的法律、法规，不从事任何危害中国秩序的、与本人来华学习身份不相符的活动； During my stay in
            China, I shall abide by the laws and decrees of the Chinese government, and will not participate in any activities,
            which are deemed to be adverse to the social order in China and inappropriate to the capacity as a scholar or a
            student.
           </p>
           <p>
            3． 在学期间，遵守学校的校纪、校规，全力投入学习和研究工作。尊重学校的教学安排； During my study in China, I shall
            observe the rules and regulations of the university, and will concentrate on my studies and researches, and will
            follow the teaching programs made by the university.
           </p>
           <p>
            4． 按规定修完学业，按期回国，不无故在华滞留； I shall return to my home country as soon as I complete my scheduled
            program in China, and will not extend my stay without valid reasons.
           </p>
           <p>
            5． 我一到学校就想办理所有登记手续。我将在抵达后30天内向公安局申请居住证，否则我必须支付延误的罚款。 I’d like to go
            through all the procedures of registration to the university as soon as I arrive. I will apply within thirty days of
            arrival for the residency permit to the Public Security Bureau otherwise I have to pay the forfeit for the delay.
            <p>
             申请人签名/Signature of the applicant:____________
             <br />
             日期/Date:_____________ 无此签名，申请无效/The application is invalid without the applicant’s signature
            </p>
           </p>
           ...
           <p>
            ...
            <p>（1） 护照复印件/One photocopy of your passport(photo page)</p>
            <p>（2） 最后学历证明/An official certificate of your highest education (or notarized photocopy)</p>
            <p>（3） 学习成绩单/Official academic transcripts (or notarized photocopy)</p>
            <p>（4） 推荐信/Two letters of recommendation (original)</p>
            <p>（5） 家庭收入证明/ An official Certificate of your sponsor’s true family financial situation;</p>
            <p>
             （6） 健康证明/ Certificate of your health examination records 无论申请人是否被录取，上述申请材料恕不退还 Whether the
             candidate is accepted or not, all the application materials will not be returned
            </p>
            <b className="font-semibold text-lg">
             Notice: Please fill in this form in capital letter. Duplication of this form is available.
            </b>
            <p>
             注意事项/Note: 申请人须用中文填写或用英文印刷体填写，其它文字或缺项填写的申请表无效。 This form is to be completed in
             Chinese or English (print) only. An incomplete application or complete in language other than Chinese or English is
             invalid.
            </p>
           </p>
          </div>
       ...
       <div className="flex items-center justify-between pt-4">
        <button type="button" onClick={reset} className="text-sm text-gray-600 underline cursor-pointer">
         Reset form
        </button>
        <div className="flex gap-3">
         {step > 0 && (
          <button type="button" onClick={back} className="rounded-2xl border px-4 py-2 text-sm font-medium hover:bg-gray-50">
           Back
          </button>
         )}
         {step < STEPS.length - 1 ? (
          <button
           ...
           className="rounded-2xl bg-[#0061a1] px-5 py-2 text-sm font-semibold text-white shadow hover:bg-blue-700"
          >
           Continue
          </button>
         ) : (
          <button
           type="submit"
           disabled={loading}
           className="rounded-2xl bg-green-600 px-5 py-2 text-sm font-semibold text-white shadow hover:bg-green-700"
          >
...
function labelFor(field) {
 const labels = {
  majorFirstChoice: "Major First Choice",
