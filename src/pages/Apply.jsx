								...
								<section className="space-y-4">
												...
												{data.educationHistory.map((edu, i) => (
													...
													>
														...
														<div className="text-xs text-gray-500">
															{edu.location ? `${edu.location} • ${edu.field || ""}` : "—"}
														</div>
														<div className="text-xs text-gray-500">
															{edu.major ? `${edu.major} • ${edu.field || ""}` : "—"}
														</div>
														<div className="text-xs text-gray-500">
															{edu.dateAttended ? `${edu.dateAttended} • ${edu.field || ""}` : "—"}
														</div>
														<div className="text-xs text-gray-500">
															{edu.degreeAwarded ? `${edu.degreeAwarded} • ${edu.field || ""}` : "—"}
														</div>
													</div>
												))}
											</div>
											...
											<div className="grid grid-cols-1 gap-3">
												{data.familyMembers.map((fam, i) => (
													...
													>
														...
														<div className="text-xs text-gray-500">
															{fam.nationality ? `${fam.nationality} • ${fam.field || ""}` : "—"}
														</div>
														<div className="text-xs text-gray-500">
															{fam.mobileNumber ? `${fam.mobileNumber} • ${fam.field || ""}` : "—"}
														</div>
														<div className="text-xs text-gray-500">
															{fam.email ? `${fam.email} • ${fam.field || ""}` : "—"}
														</div>
														<div className="text-xs text-gray-500">
															{fam.profession ? `${fam.profession} • ${fam.field || ""}` : "—"}
														</div>
														<div className="text-xs text-gray-500">
															{fam.workPlace ? `${fam.workPlace} • ${fam.field || ""}` : "—"}
														</div>
														<div className="text-xs text-gray-500">
															{fam.relationship ? `${fam.relationship} • ${fam.field || ""}` : "—"}
														</div>
													</div>
												))}
											</div>
									...
									{submitted && (
										<div className="rounded-xl border border-green-200 bg-gray-50 p-4 text-black">
											...
											<p>
												1． 申请表中所填写的内容和提供的材料真实无误； All information and materials given in this
											...
											<p>
												...
												China and inappropriate to the capacity as a scholar or a student.
											</p>
											<p>
												3． 在学期间，遵守学校的校纪、校规，全力投入学习和研究工作。尊重学校的教学安排； During my
												study in China, I shall observe the rules and regulations of the university, and will
												concentrate on my studies and researches, and will follow the teaching programs made by the
												university.
											</p>
											<p>
												4． 按规定修完学业，按期回国，不无故在华滞留； I shall return to my home country as soon as I
												complete my scheduled program in China, and will not extend my stay without valid reasons.
											</p>
											<p>
												5．
												我一到学校就想办理所有登记手续。我将在抵达后30天内向公安局申请居住证，否则我必须支付延误的罚款。
												I’d like to go through all the procedures of registration to the university as soon as I
												arrive. I will apply within thirty days of arrival for the residency permit to the Public
												Security Bureau otherwise I have to pay the forfeit for the delay.
												<p>
													申请人签名/Signature of the applicant:____________
													<br />
													日期/Date:_____________ 无此签名，申请无效/The application is invalid without the
											...
											<p>
												...
												<p>
													（2） 最后学历证明/An official certificate of your highest education (or notarized
													photocopy)
												</p>
												<p>（3） 学习成绩单/Official academic transcripts (or notarized photocopy)</p>
												<p>（4） 推荐信/Two letters of recommendation (original)</p>
												<p>
													（5） 家庭收入证明/ An official Certificate of your sponsor’s true family financial
													situation;
												</p>
												<p>
													（6） 健康证明/ Certificate of your health examination records
													无论申请人是否被录取，上述申请材料恕不退还 Whether the candidate is accepted or not, all
												...
												<p>
													注意事项/Note: 申请人须用中文填写或用英文印刷体填写，其它文字或缺项填写的申请表无效。 This
													form is to be completed in Chinese or English (print) only. An incomplete application or
													complete in language other than Chinese or English is invalid.
												</p>
											</p>
										</div>
									)}
